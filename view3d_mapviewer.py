...
####################################
#Custom basemap to eliminate the need for a view area
class HeadlessBaseMap(GeoScene):
	...
	def run(self):
		"""thread method"""
		self.mosaic = self.request() ##bound to area
		if self.srv.running and self.mosaic is not None:
			#save image
			self.mosaic.save(self.imgPath)
		if self.srv.running:
	...
	def request(self):
		...
		##Custom statics during dev
		##Crs is defined previously by the address search
		pxRes = 2048
		w, h = pxRes, pxRes
		z = 18
		##Centered around the crs coords
...
class VIEW3D_OT_map_headless_export(Operator):

	bl_idname = "view3d.map_headless_export"
	bl_description = 'Exports a region without the gui'
	bl_label = "Map export"
	bl_options = {'INTERNAL'}
	...
	def init(self, context):
		...
		print("init OK up to Basemap")
		
		#Get map
		self.map = HeadlessBaseMap(context, self.srckey, self.laykey, self.grdkey)
		
		#zoom here is either already supplied by the scene, or set to 18
		##if self.recenter and len(context.scene.objects) > 0:
		##	scnBbox = getBBOX.fromScn(context.scene).to2D()
		##	w, h = scnBbox.dimensions
		##	px_diag = math.sqrt(context.area.width**2 + context.area.height**2)
